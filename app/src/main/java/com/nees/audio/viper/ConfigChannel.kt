...
object ConfigChannel {
    ...
    fun ensureInitialized() {
        ...
        synchronized(writeLock) {
            ...
            try {
                ...
                FileLogger.w("ConfigChannel", "Direct mmap failed; preparing SHM")
                createShmViaSu()
                if (mapShm()) {
                    initDone.set(true)
                    return
                }
                if (RootShmChannel.ensureInitialized()) {
                    FileLogger.i("ConfigChannel", "Using root-backed SHM control plane")
                    initDone.set(true)
                }
            } catch (e: Exception) {
    ...
    fun writeFullState(state: EffectState) {
        ...
        synchronized(writeLock) {
            ...
            val buf = paramsBuffer
            if (buf == null) {
                currentUpdateCount++
                if (RootShmChannel.writeFullState(state, nextSlot, currentUpdateCount)) {
                    producerActiveSlot = nextSlot
                } else {
                    currentUpdateCount--
                    FileLogger.w("ConfigChannel", "writeFullState: root SHM write failed")
                }
                return
            }
            val slotOffset =
    ...
    fun writeBulkDdc(
        ...
        synchronized(writeLock) {
            ...
            if (buf == null) {
                currentDdcSeq++
                if (!RootShmChannel.writeDdc(perRateFloats, coeffs, currentDdcSeq)) {
                    currentDdcSeq--
                    FileLogger.w("ConfigChannel", "writeBulkDdc: root SHM write failed")
                }
                return
            }
    ...
    fun writeBulkConvolverPath(path: String) {
        ...
        synchronized(writeLock) {
            ...
            if (buf == null) {
                currentConvolverSeq++
                if (!RootShmChannel.writeConvolverPath(path, currentConvolverSeq)) {
                    currentConvolverSeq--
                    FileLogger.w("ConfigChannel", "writeBulkConvolverPath: root SHM write failed")
                }
                return
            }
